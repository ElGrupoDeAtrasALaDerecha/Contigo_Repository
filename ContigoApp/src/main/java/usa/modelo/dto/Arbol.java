...
public class Arbol {
    ...
    private Situacion primerNodo;

    /**
     * Método que agrega una situación a un árbol (o un final). Se agregan nodos
     * con necesidad de encontrar un nodo padre para componer la historia.
     *
     * @param situacion que es la situación de decisión que se agregará al árbol
     */
    public void agregarNodo(IComponente situacion) {
        if (primerNodo == null) {
     ...
     * @param situacion que es una situación que posiblemente sea el padre o que
     * contenga al padre
    ...
    public Componente buscarPadre(int id, Situacion situacion) {
        ...
        LinkedList<IComponente> situaciones = situacion.getOpciones();
        for (int i = 0; i < situaciones.size(); i++) {
            if (situaciones.get(i).getId() == id) {
                return (Componente)situaciones.get(i);
            } else {
    ...
    public Situacion getPrimerNodo() {
        return primerNodo;
    }
