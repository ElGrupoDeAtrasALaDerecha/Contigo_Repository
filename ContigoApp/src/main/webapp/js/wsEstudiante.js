	...
	websocket.onmessage = function (event) {
		if (event.data === "pong") {
			...
			if (obj.tipo === "codigo sala") {
				...
				console.log(obj.personal)
				if(obj.personal!==undefined){
					pintarPersonal(obj.personal);
				}
				if(obj.tipo==="perdidaConexion"){
					pintarConti();
				}
				pintarRespuesta(obj.mensaje);
...
function pintarConti(){
	...
	$("#titulo").find("h1").remove();
	$("#titulo").prepend(`<h1>Conti</h1>`);
	$("#logo").append(`<img id="pin" src="Iconos/CONTI.png">`);
}